...
2\. \*\*上下文与持久性\*\*：严格遵守 `<context\_gathering>`、`<persistence>`、`<self\_reflection>` 等标签约束
...
\- \*\*多步任务（≥2 步）必须使用 `update\_plan` 工具追踪进度\*\*

\- 实时更新状态：`pending` → `in\_progress` → `completed`
...
&nbsp; - 文件建议命名为：`plan/YYYY-MM-DD\_HH-mm-ss-<slug>.md`，其中：

&nbsp;   - 时间戳可通过当前系统可用的方式获取：

&nbsp;     - 类 Unix 环境：例如 `date +"%Y-%m-%d\_%H-%M-%S"`；

&nbsp;     - Windows PowerShell：例如 `Get-Date -Format "yyyy-MM-dd\_HH-mm-ss"`；
...
&nbsp; planning\_method: builtin

&nbsp; created\_at: <ISO8601 时间戳或 date 输出>
...
&nbsp; - 要求在 frontmatter 中记录 `planning\_method: builtin`（并包含 `task` / `complexity` / `created\_at` 等字段）；

&nbsp; - 推荐通过脚本周期性检查 `plan/\*.md` 是否满足该约定（例如 grep 检查 `planning\_method:` 字段）；
...
\- \*\*内置推理（默认）\*\*：复杂规划无需额外 MCP；先内部拆解，再输出计划并用 `update\_plan` 跟踪进度
...
\- \[ ] 已记录 ≥2 步计划，使用 `update\_plan` 追踪进度

\- \[ ] 验证包括测试/检查及 `<self\_reflection>` 自评
